...
if [ $# -ne 2 ];
then
  echo "[:] Put usernames and terms to search in their respective .txt files and run the script:"
  echo "[:] Usage: $0 usernames.txt searchterms.txt"
  exit -1
fi
#-- ask user to confirm start of script, define variable $resp:
echo ""
read -p "[:] twint batch user tweetgrabber & search plus html output script, shall we begin? (y/n) " resp
echo ""
...
if [ "$resp" = "y" ]; then
  ...
  #-- define variable $search as each line in a file $2, and start for loop:
  search=$(cat ../$2)
  for terms in $search;
  do
    ...
    do
      ...
      echo "[:] searching username $line, searchterm is $terms..."
  ...
  echo ""
#-- if the user enters anything but "y", exit to shell:
else
  echo "[:] exiting..."
  exit -1
fi