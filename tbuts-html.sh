...
if [ "$resp" = "y" ]; then
	...
	for line in $lines;
		do
			...
			echo "</article></ul></body></html>" >> ~/tbuts-html/results/$line-tweets-$2.html
			#--tell user where the output(s) are located:
			echo ".txt output is ~/tbuts-html/results/$line-$2.txt"
			echo ".html output is ~/tbuts-html/results/$line-tweets-$2.html"
			echo ""
		#--loop complete:
		done
